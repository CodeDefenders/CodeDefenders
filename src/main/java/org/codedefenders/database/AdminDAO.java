...
public class AdminDAO {
    ...
    public static List<List<String>> getUnassignedUsersInfo() throws UncheckedSQLException, SQLMappingException {
        final String query = String.join("\n",
                ...
                "    LEFT JOIN mutants mutants ON players.id = mutants.Player_ID",
                "    GROUP BY players.User_ID",
                ...
                "    FROM players players",
                "    LEFT JOIN tests tests ON players.id = tests.Player_ID",
                "    GROUP BY players.User_ID",
                ...
                "      AND games.Mode <> 'PUZZLE'",
                "      AND (State = 'ACTIVE' OR State = 'CREATED')",
                "      AND Role IN ('ATTACKER', 'DEFENDER')",
                "      AND Active = 1",
                ")",
