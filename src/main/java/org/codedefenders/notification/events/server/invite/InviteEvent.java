/*
 * Copyright (C) 2016-2025 Code Defenders contributors
 *
 * This file is part of Code Defenders.
 *
 * Code Defenders is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or (at
 * your option) any later version.
 *
 * Code Defenders is distributed in the hope that it will be useful, but
 * WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU
 * General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with Code Defenders. If not, see <http://www.gnu.org/licenses/>.
 */
package org.codedefenders.notification.events.server.invite;

import org.codedefenders.model.WhitelistType;
import org.codedefenders.notification.events.server.ServerEvent;

import com.google.gson.annotations.Expose;

public class InviteEvent extends ServerEvent {
    @Expose
    private int userId;

    @Expose
    private int gameId;

    @Expose
    private boolean mayChooseRole;

    @Expose
    private String inviteLink;

    @Expose
    private String className;

    @Expose
    private String role;

    public int getUserId() {
        return userId;
    }

    public void setUserId(int userId) {
        this.userId = userId;
    }

    public boolean isMayChooseRole() {
        return mayChooseRole;
    }

    public void setMayChooseRole(boolean mayChooseRole) {
        this.mayChooseRole = mayChooseRole;
    }

    public String getInviteLink() {
        return inviteLink;
    }

    public void setInviteLink(String inviteLink) {
        this.inviteLink = inviteLink;
    }

    public String getClassName() {
        return className;
    }

    public void setClassName(String className) {
        this.className = className;
    }

    public WhitelistType getRole() {
        return WhitelistType.valueOf(role);
    }

    public void setRole(WhitelistType role) {
        this.role = role.name();
    }
}
