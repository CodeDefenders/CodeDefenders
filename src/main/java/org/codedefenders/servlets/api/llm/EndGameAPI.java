...
@WebServlet("/llm-api/battleground/end")
public class EndGameAPI extends APIServlet {
    @Inject
    ...
    protected void doPost(HttpServletRequest request,
        ...
        if (game.getCreatorId() != login.getUserId() && game.getRole(login.getUserId()) != Role.OBSERVER) {
            ...
            writeResponse(response, HttpServletResponse.SC_FORBIDDEN,
                    new Common.ErrorResponseDTO("Only the game's creator or an observer can end the game."));
            return;
        }

        if (game.getState() == GameState.ACTIVE) {
            gameService.closeGame(game);
            writeResponse(response, HttpServletResponse.SC_OK, new EndGameResponseDTO(true, "Game ended."));
        } else {
            writeResponse(response, HttpServletResponse.SC_EXPECTATION_FAILED, new EndGameResponseDTO(false,
                    "Game has to be in ACTIVE state to be ended."));
        }

    }

    public record EndGameResponseDTO(boolean success, String message) {}
}
