...
public class DatabaseAccess {
	...
	/**
	 * Sanitises user input. If a whole SQL query is entered, syntax errors may
	 * occur.
	 * 
	 * @param s
	 *            user input String
...
//		System.out.println("DatabaseAccess.getConnection() "
//				+ Thread.currentThread().getName() + " got a connection ");
...
//		System.out.println("DatabaseAccess.cleanup() "
//				+ Thread.currentThread().getName() + " release the connection");
	}
	...
	public static void insertMessage(int uid, String ipAddress) {
		...
		try {
			...
			String sql = String.format(
					"INSERT INTO events (User_ID, IP_Address) VALUES ('%d', '%s');",
					uid, ipAddress);
	...
	public static ArrayList<Event> getEventsForGame(int gameId) {
		String sql = String.format(
				"SELECT * FROM events "
						...
						+ "WHERE Game_ID='%d' " + "AND Event_Status='%s'",
	...
	public static void removePlayerEventsForGame(int gameId, int playerId) {
		String sql = String.format(
				"SELECT * FROM events WHERE Game_ID=%d " + "AND Player_ID=%d",
				gameId, EventStatus.GAME);
	...
	public static ArrayList<Event> getEventsForUser(int userId) {
		String sql = String.format("SELECT * FROM events "
				+ "LEFT JOIN event_messages AS em ON events.Event_Type = em.Event_Type "
				+ "LEFT JOIN event_chat AS ec ON events.Event_Id = ec.Event_Id "
				+ "WHERE " + "Event_Status!='DELETED' " + "AND Player_ID='%d';",
				userId);
		return getEvents(sql);
	}
	...
	public static ArrayList<Event> getNewEventsForUser(int userId, long time) {
		String sql = String.format(
				"SELECT * FROM events "
						...
						+ "LEFT JOIN event_chat AS ec ON events.Event_Id = ec.Event_Id "
						+ "WHERE Player_ID='%d'" + " "
						+ "AND Event_Status<>'%s' " + "AND Event_Status<>'%s' "
						+ "AND Timestamp >= FROM_UNIXTIME(%d)",
	...
	public static void setGameAsAIDummy(int gId) {
		String sql = String.format(
				"UPDATE games SET IsAIDummyGame = 1 WHERE ID = %d;", gId);
		executeUpdate(sql);
	}

	public static DuelGame getAiDummyGameForClass(int cId)
			throws NoDummyGameException {
		String sql = String.format(
				"SELECT * FROM games WHERE Class_ID='%d' AND IsAIDummyGame=1",
				cId);
		int gID = getInt(sql, "ID");
		if (gID == 0) {
			NoDummyGameException e = new NoDummyGameException(
					"No dummy game found.");
			throw e;
		}
	...
	public static int getNumAiMutantsKilledByTest(int tId) {
		String sql = String.format("SELECT * FROM tests WHERE Test_ID='%d';",
				tId);
	...
	public static int getNumTestsKillMutant(int mId) {
		String sql = String
				.format("SELECT * FROM mutants WHERE Mutant_ID='%d';", mId);
	...
	public static GameClass getClassForGame(int gameId) {
		String sql = String.format(
				"SELECT classes.* from classes INNER JOIN games ON classes.Class_ID = games.Class_ID WHERE games.ID=%d;",
				gameId);
		return getClass(sql);
	}

	public static GameClass getClassForKey(String keyName, int id) {
		String sql = String.format("SELECT * FROM classes WHERE %s=%d;",
				keyName, id);
		return getClass(sql);
	}
	...
	public static boolean isAiPrepared(GameClass c) {
		...
		Statement stmt = null;
		String sql = String.format(
				"SELECT * from classes WHERE AiPrepared = 1 AND Class_ID = %d",
				c.getId());
	...
	public static void setAiPrepared(GameClass c) {
		String sql = String.format(
				"UPDATE classes SET AiPrepared = 1 WHERE Class_ID = %d;",
				c.getId());
		executeUpdate(sql);
	}
		...
		try {
			...
			while (rs.next()) {
				Event event = new Event(rs.getInt("events.Event_ID"),
						rs.getInt("Game_ID"), rs.getInt("Player_ID"),
	...
	private static GameClass getClass(String sql) {
		...
		try {
			...
			if (rs.next()) {
				GameClass classRecord = new GameClass(rs.getInt("Class_ID"),
	...
	public static List<GameClass> getAllClasses() {
		...
		try {
			...
			while (rs.next()) {
				classList.add(new GameClass(rs.getInt("Class_ID"),
						rs.getString("Name"), rs.getString("Alias"),
						rs.getString("JavaFile"), rs.getString("ClassFile")));
			}
		} catch (SQLException se) {
	...
	public static List<User> getAllUsers() {
		...
		try {
			...
			while (rs.next()) {
				User userRecord = new User(rs.getInt("User_ID"),
						rs.getString("Username"), rs.getString("Password"),
						rs.getString("Email"), rs.getBoolean("Validated"));
	...
	public static User getUserForNameOrEmail(String usernameOrEmail) {
		String sql = String.format(
				"SELECT * FROM users WHERE Username='%1$s' OR Email='%1$s';",
				usernameOrEmail);
	...
	public static User getUserForKey(String keyName, int id) {
		String sql = String.format("SELECT * FROM users WHERE %s=%d;", keyName,
				id);
		return getUserFromDB(sql);
	}
	...
	public static DuelGame getGameForKey(String keyName, int id) {
		...
		try {
			...
			stmt = conn.createStatement();
			sql = String
					.format("SELECT g.ID, g.Class_ID, g.Level, g.Creator_ID, g.State,"
			...
			if (rs.next()) {
				...
				if (rs.getString("Mode").equals(GameMode.SINGLE.name()))
					gameRecord = new SinglePlayerGame(rs.getInt("ID"),
							rs.getInt("Attacker_ID"), rs.getInt("Defender_ID"),
							rs.getInt("Class_ID"), rs.getInt("CurrentRound"),
							rs.getInt("FinalRound"),
							Role.valueOf(rs.getString("ActiveRole")),
							GameState.valueOf(rs.getString("State")),
							GameLevel.valueOf(rs.getString("Level")),
							GameMode.valueOf(rs.getString("Mode")));
				else
					gameRecord = new DuelGame(rs.getInt("ID"),
							rs.getInt("Attacker_ID"), rs.getInt("Defender_ID"),
							rs.getInt("Class_ID"), rs.getInt("CurrentRound"),
							rs.getInt("FinalRound"),
							Role.valueOf(rs.getString("ActiveRole")),
	...
	public static List<DuelGame> getGamesForUser(int userId) {
		...
		String sql = String.format(
				"SELECT g.ID, g.Class_ID, g.Level, g.Creator_ID, g.State,"
						+ "g.CurrentRound, g.FinalRound, g.ActiveRole, g.Mode, g.Creator_ID,\n"
						+ "IFNULL(att.User_ID,0) AS Attacker_ID, IFNULL(def.User_ID,0) AS Defender_ID\n"
						+ "FROM games as g\n"
						+ "LEFT JOIN players as att ON g.ID=att.Game_ID  AND att.Role='ATTACKER' AND att.Active=TRUE\n"
	...
	public static List<MultiplayerGame> getJoinedMultiplayerGamesForUser(
			int userId) {
	...
	public static List<MultiplayerGame> getFinishedMultiplayerGamesForUser(
			int userId) {
		String sql = String.format("SELECT * FROM games AS m "
				+ "LEFT JOIN players as p ON p.Game_ID=m.ID  AND p.Active=TRUE \n"
	...
	public static List<MultiplayerGame> getOpenMultiplayerGamesForUser(
			int userId) {
		String sql = String.format(
	...
	public static DuelGame getActiveUnitTestingSession(int userId) {
		String sql = String.format(
				"SELECT * FROM games WHERE Defender_ID='%d' AND Mode='UTESTING' AND State='ACTIVE';",
				userId);
	...
	public static List<DuelGame> getGames(String sql) {
		...
		try {
			...
			while (rs.next()) {
				gameList.add(new DuelGame(rs.getInt("ID"),
						rs.getInt("Attacker_ID"), rs.getInt("Defender_ID"),
						rs.getInt("Class_ID"), rs.getInt("CurrentRound"),
						rs.getInt("FinalRound"),
						Role.valueOf(rs.getString("ActiveRole")),
	...
	public static MultiplayerGame getMultiplayerGame(int id) {
		String sql = String.format(
				"SELECT * FROM games AS m " + "WHERE ID=%d AND m.Mode='PARTY'",
				id);
	...
	public static List<MultiplayerGame> getMultiplayerGames(String sql) {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery(sql);
			while (rs.next()) {
				MultiplayerGame mg = new MultiplayerGame(rs.getInt("Class_ID"),
						rs.getInt("Creator_ID"),
						GameLevel.valueOf(rs.getString("Level")),
						(float) rs.getDouble("Coverage_Goal"),
						(float) rs.getDouble("Mutant_Goal"), rs.getInt("Prize"),
						rs.getInt("Defender_Value"),
						rs.getInt("Attacker_Value"),
						rs.getInt("Defenders_Limit"),
						rs.getInt("Attackers_Limit"),
						rs.getInt("Defenders_Needed"),
						rs.getInt("Attackers_Needed"),
						rs.getTimestamp("Start_Time").getTime(),
						rs.getTimestamp("Finish_Time").getTime(),
						rs.getString("State"),
						rs.getBoolean("RequiresValidation"));
	...
	public static List<Mutant> getMutantsForGame(int gid) {
		...
		try {
			...
			stmt = conn.createStatement();
			sql = String.format(
					"SELECT * FROM mutants WHERE Game_ID='%d' AND ClassFile IS NOT NULL;",
					gid);
			ResultSet rs = stmt.executeQuery(sql);

			while (rs.next()) {
				Mutant newMutant = new Mutant(rs.getInt("Mutant_ID"),
						rs.getInt("Game_ID"), rs.getString("JavaFile"),
						rs.getString("ClassFile"), rs.getBoolean("Alive"),
						Mutant.Equivalence.valueOf(rs.getString("Equivalent")),
	...
	public static List<Mutant> getMutantsForPlayer(int pid) {
		...
		try {
			...
			stmt = conn.createStatement();
			sql = String.format(
					"SELECT * FROM mutants WHERE Player_ID='%d' AND ClassFile IS NOT NULL;",
					pid);
			ResultSet rs = stmt.executeQuery(sql);
	...
	public static Mutant getMutantFromDB(String sql) {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery(sql);
			if (rs.next()) {
				newMutant = new Mutant(rs.getInt("Mutant_ID"),
						rs.getInt("Game_ID"), rs.getString("JavaFile"),
						rs.getString("ClassFile"), rs.getBoolean("Alive"),
						Mutant.Equivalence.valueOf(rs.getString("Equivalent")),
						rs.getInt("RoundCreated"), rs.getInt("RoundKilled"),
	...
	public static Mutant getMutant(DuelGame game, int mutantID) {
		String sql = String.format(
				"SELECT * FROM mutants WHERE Mutant_ID='%d' AND Game_ID='%d';",
				mutantID, game.getId());
	...
	public static Mutant getMutant(int gameId, String md5) {
		String sql = String.format(
				"SELECT * FROM mutants WHERE Game_ID='%d' AND MD5='%s';",
				gameId, md5);
		return getMutantFromDB(sql);
	}
	...
	public static List<Integer> getUsedAiTestsForGame(DuelGame g) {
		...
		try {
			...
			stmt = conn.createStatement();
			String sql = String.format(
					"SELECT * FROM usedaitests WHERE Game_ID='%d';", g.getId());
			ResultSet rs = stmt.executeQuery(sql);
	...
	public static void increasePlayerPoints(int points, int player) {
		String sql = String.format(
				"UPDATE players SET Points=Points+%d WHERE ID=%d", points,
				player);
		execute(sql);
	}
	...
	public static int getEquivalentDefenderId(Mutant m) {
		...
		try {
			...
			stmt = conn.createStatement();
			String sql = String.format(
					"SELECT * FROM equivalences WHERE Mutant_ID='%d';",
					m.getId());
			ResultSet rs = stmt.executeQuery(sql);
			int id = -1;
			while (rs.next()) {
	...
	public static int getPlayerPoints(int playerId) {
		...
		try {
			conn = getConnection();
			stmt = conn.createStatement();
			String sql = String.format("SELECT * FROM players WHERE ID='%d';",
					playerId);
			ResultSet rs = stmt.executeQuery(sql);
			int points = 0;
			while (rs.next()) {
	...
	public static boolean setAiTestAsUsed(int testNumber, DuelGame g) {
		...
		try {
			...
			stmt = conn.createStatement();
			String sql = String.format(
					"INSERT INTO usedaitests (Value, Game_ID) VALUES ('%d', '%d');",
					testNumber, g.getId());
	...
	public static List<Integer> getUsedAiMutantsForGame(DuelGame g) {
		...
		try {
			...
			stmt = conn.createStatement();
			String sql = String.format(
					"SELECT * FROM usedaimutants WHERE Game_ID='%d';",
					g.getId());
			ResultSet rs = stmt.executeQuery(sql);
	 ...
	 *
	 * @param mutantNumber
	 *            the number of the mutant
	 * @param g
	 *            the game the mutant belongs to
	 * @return
	 */
	public static boolean setAiMutantAsUsed(int mutantNumber, DuelGame g) {
		...
		try {
			...
			stmt = conn.createStatement();
			String sql = String.format(
					"INSERT INTO usedaimutants (Value, Game_ID) VALUES ('%d', '%d');",
					mutantNumber, g.getId());
	...
	public static List<Test> getTestsForGame(int gid) {
		String sql = String.format("SELECT * FROM tests WHERE Game_ID='%d';",
				gid);
		return getTests(sql);
	}
	 ...
	 */
	public static List<Test> getExecutableTests(int gameId,
			boolean defendersOnly) {
		String stmt = "SELECT tests.* FROM tests\n"
				...
				+ (defendersOnly ? "AND pl.Role='DEFENDER'\n" : "") // [optional]
																	...
																	// defenders
																	// only
				+ "AND ex.Target='TEST_ORIGINAL' AND ex.Status='SUCCESS';"; // that
																			// pass
																			// on
																			// original
																			// CUT
	...
	public static int getPlayerIdForMultiplayerGame(int userId, int gameId) {
		String sql = String.format(
				"SELECT * FROM players AS p "
						+ "WHERE p.User_ID = %d AND p.Game_ID = %d",
				userId, gameId); // that pass on original CUT
	...
	public static int[] getPlayersForMultiplayerGame(int gameId, Role role) {
		...
		String sql = String.format(
				"SELECT * FROM players \n"
						+ "WHERE Game_ID = %d AND Role='%s' AND Active=TRUE;",
				gameId, role);
	...
	private static List<Test> getTests(String sql) {
		...
		try {
			...
			while (rs.next()) {
				Test newTest = new Test(rs.getInt("Test_ID"),
						rs.getInt("Game_ID"), rs.getString("JavaFile"),
						rs.getString("ClassFile"), rs.getInt("RoundCreated"),
						rs.getInt("MutantsKilled"), rs.getInt("Player_ID"));
				String lcs = rs.getString("Lines_Covered");
				String lucs = rs.getString("Lines_Uncovered");
				LineCoverage lc = new LineCoverage();
				if (lcs != null && lucs != null && lcs.length() > 0
						&& lucs.length() > 0) {
	...
	public static int getKillingTestIdForMutant(int mid) {
		String sql = String.format(
				"SELECT * FROM targetexecutions WHERE Target='TEST_MUTANT' AND Status!='SUCCESS' AND Mutant_ID='%d';",
				mid);
		TargetExecution targ = getTargetExecutionSQL(sql);
		return (targ != null) ? targ.testId : -1; // TODO: We shouldn't give
													// away that we don't know
													// which test killed the
													// mutant?
	}

	public static TargetExecution getTargetExecutionForPair(int tid, int mid) {
		String sql = String.format(
				"SELECT * FROM targetexecutions WHERE Test_ID='%d' AND Mutant_ID='%d';",
				tid, mid);
		return getTargetExecutionSQL(sql);
	}

	public static TargetExecution getTargetExecutionForTest(Test test,
			TargetExecution.Target target) {
		String sql = String.format(
				"SELECT * FROM targetexecutions WHERE Test_ID='%d' AND Target='%s';",
				test.getId(), target.name());
		return getTargetExecutionSQL(sql);
	}

	public static TargetExecution getTargetExecutionForMutant(Mutant mutant,
			TargetExecution.Target target) {
		String sql = String.format(
				"SELECT * FROM targetexecutions WHERE Mutant_ID='%d' AND Target='%s';",
				mutant.getId(), target.name());
		return getTargetExecutionSQL(sql);
	}
		...
		try {
			...
			if (rs.next()) {
				TargetExecution targetExecution = new TargetExecution(
						rs.getInt("TargetExecution_ID"), rs.getInt("Test_ID"),
						rs.getInt("Mutant_ID"),
						TargetExecution.Target.valueOf(rs.getString("Target")),
	...
	public static void logSession(int uid, String ipAddress) {
		...
		try {
			...
			stmt = conn.createStatement();
			String sql = String.format(
					"INSERT INTO sessions (User_ID, IP_Address) VALUES ('%d', '%s');",
					uid, ipAddress);
