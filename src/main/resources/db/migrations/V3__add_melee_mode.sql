...
ALTER TABLE `players`
    MODIFY COLUMN `Role` enum ('ATTACKER','DEFENDER','PLAYER') NOT NULL;
ALTER TABLE `games`
    MODIFY COLUMN `ActiveRole` ENUM ('ATTACKER','DEFENDER','PLAYER') NOT NULL DEFAULT 'ATTACKER';

/* Add new game mode */
ALTER TABLE `games`
    MODIFY COLUMN `Mode` enum ('SINGLE','DUEL','PARTY','UTESTING','PUZZLE', 'MELEE') NOT NULL DEFAULT 'PARTY';

/* Add new events for melee mode*/
INSERT INTO `event_messages`
VALUES ('PLAYER_JOINED', '@event_user joined the game'),
