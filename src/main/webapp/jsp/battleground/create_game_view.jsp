...
<div id="creategame" class="container">
    <p>
        Before you can start games, please <a href="<%=request.getContextPath() + Paths.CLASS_UPLOAD%>?origin=<%=Paths.BATTLEGROUND_CREATE%>" class="text-center new-account">upload a class under
        test</a>.
    </p>
</div>
<%
    ...
    <form id="create" action="<%=request.getContextPath()  + Paths.BATTLEGROUND_SELECTION%>" method="post"
        ...
        <table style="border-collapse: separate; border-spacing: 20px; font-size: 16px; text-align: center; line-height: 27px;">
            <tr>
                ...
                <%if (isClassUploadEnabled) {%>
                    <td width="17%">
                        <a href="<%=request.getContextPath() + Paths.CLASS_UPLOAD%>?origin=<%=Paths.BATTLEGROUND_CREATE%>" class="text-center new-account">Upload Class</a>
                    </td>
                <%}%>
            </tr>
            <!--
